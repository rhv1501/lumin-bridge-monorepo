      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {isLoading ? (
          Array.from({ length: 8 }).map((_, i) => (
            <Card
              key={i}
              className="overflow-hidden border-0 shadow-lg bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl"
            >
          ...
          products.map((p) => (
            <Card
              key={p.id}
              className="overflow-hidden group hover:shadow-xl transition-all duration-500 border-0 shadow-lg bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl flex flex-col h-full rounded-2xl"
            >
              <div className="aspect-[4/3] bg-zinc-100 dark:bg-zinc-800 relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => onEdit(p)}
                    className="p-2 bg-white/90 dark:bg-zinc-900/90 rounded-full hover:bg-white text-zinc-600"
                  >
                    <Edit size={14} />
                  </button>
                  <button
                    onClick={() => onDelete(p.id)}
                    className="p-2 bg-white/90 dark:bg-zinc-900/90 rounded-full hover:bg-white text-red-600"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
                <div className="absolute top-3 left-3">
                  <span
                    className={cn(
                      ...
                      p.status === "published"
                        ? "bg-emerald-500 text-zinc-950 dark:text-white"
                        : "bg-amber-500 text-zinc-950 dark:text-white",
                    )}
                  >
                    {p.status}
                  </span>
              ...
              <div className="p-5 flex flex-col flex-1">
                <div className="flex justify-between items-start mb-2 gap-2">
                  <h3 className="font-semibold text-base tracking-tight text-zinc-900 dark:text-white line-clamp-1">
                    {p.name}
                  </h3>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800/50 mt-auto">
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold uppercase text-zinc-500 dark:text-zinc-400 mb-0.5">
                      Price
                    </span>
                    <span className="font-mono font-semibold text-base text-zinc-900 dark:text-white">
                      ¥{p.factory_price_cny}
                    </span>
                  </div>
