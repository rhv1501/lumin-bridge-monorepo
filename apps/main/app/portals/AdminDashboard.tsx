...
export const AdminDashboard = ({
  ...
  const adminTabs = [
    ...
    { id: "products", label: "Products", icon: Package },
    {
      ...
      label: "Orders",
      icon: ShoppingCart,
      badge: hasNewOrders(orders),
    },
    {
      id: "custom-orders",
      label: "Custom",
      icon: FileText,
      badge: hasNewCustomOrders(customOrders, proposals),
    },
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 text-base sm:text-lg max-w-2xl">
            Global platform management and system configuration.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className="rounded-full h-11 sm:h-12 px-5 sm:px-6 text-sm"
          >
            ...
            >
              {activeTab === tab.id && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-zinc-100 dark:bg-zinc-800 rounded-3xl shadow-lg border border-zinc-200 dark:border-zinc-700"
            ...
            >
              ...
              {activeTab === "users" && (
                <div className="space-y-6">
                  <div className="relative group">
                    <Search
                      className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-500 dark:text-zinc-400"
                      size={20}
                    />
                    <Input
                  ...
                  <UserTable
                    users={users.filter(
                      (u) =>
                        ...
                        u.company_name?.includes(searchTerm),
                    )}
                    isLoading={isLoading}
                ...
                <AdminCustomOrderList
                  ...
                  onPublishProposal={async (proposalId, priceInr) => {
                    const res = await fetch(
                      `/api/custom-order-proposals/${proposalId}`,
                      {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
              ...
              {activeTab === "settings" && (
                <div className="space-y-10">
                  <ProfileSettings
                    user={user}
                    onUpdate={() => toast.success("Profile saved")}
                  />

                  <Card className="p-8 border-0 shadow-xl bg-white dark:bg-zinc-900/90 backdrop-blur-xl rounded-[2.5rem]">
                    <div className="flex items-center gap-3 mb-8">
                      <div className="w-10 h-10 bg-white dark:bg-zinc-800 rounded-2xl flex items-center justify-center">
                        <SettingsIcon
                          size={20}
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="space-y-3">
                          ...
                          />
                          <p className="text-xs text-zinc-500 dark:text-zinc-400 ml-1 italic">
                            Conversion factor for pricing calculation.
                          </p>
                        </div>
                        <div className="space-y-3">
                          <label className="text-[11px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 ml-1">
                          ...
                          />
                          <p className="text-xs text-zinc-500 dark:text-zinc-400 ml-1 italic">
                            Markup applied to factory unit prices.
                          </p>
                        </div>
                      </div>
                      <div className="flex justify-end pt-6 border-t border-zinc-200 dark:border-zinc-800">
                        <Button
                          type="submit"
                          className="h-14 px-10 rounded-2xl shadow-xl shadow-zinc-900/10"
                        >
