...
export const BuyerDashboard = ({
  ...
  const [allProposals, setAllProposals] = useState<CustomOrderProposal[]>(
    initialAllProposals || [],
  );
  ...
  // Keep local dashboard state in sync with realtime prop updates.
  useEffect(() => {
  ...
  useEffect(() => {
    setAllProposals(initialAllProposals || []);
  }, [initialAllProposals]);
  ...
  const buyerTabs = [
    ...
    },
    {
      ...
      icon: FileText,
      badge: hasNewCustomOrders(customOrders, allProposals),
    },
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-zinc-950 dark:text-white">
            Welcome, {user.company_name || "Guest"}
          </h1>
          <p className="text-zinc-500 dark:text-zinc-400 text-base sm:text-lg max-w-2xl">
            Manage your global sourcing and manufacturing orders.
          </p>
        </div>
              ...
              {activeTab === "products" && (
                <div className="space-y-8">
                  ...
                  <div className="relative group">
                    <Search
                      className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-500 dark:text-zinc-400 transition-colors group-focus-within:text-zinc-900"
                      size={20}
                    />
                    <Input
      ...
      <AnimatePresence>
        {selectedProduct && (
          <ProductModal
            ...
            onCustomRequest={() => setShowCustomOrderModal(true)}
          />
        )}
