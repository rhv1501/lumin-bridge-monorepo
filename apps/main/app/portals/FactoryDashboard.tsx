...
import { Package, ShoppingCart, FileText, Users, Search, Filter, Settings as SettingsIcon, Star } from 'lucide-react';
...
import { User, Product, Order, CustomOrder, CustomOrderProposal, Review } from "@luminbridge/types";
...
interface FactoryDashboardProps {
  ...
  activeTab: 'overview' | 'products' | 'orders' | 'custom-orders' | 'reviews' | 'settings';
...
export const FactoryDashboard = ({ 
  ...
  const handleProposalSubmit = async (customOrderId: number, data: any) => {
    try {
      ...
      <div className="grid grid-cols-12 gap-6 lg:gap-10">
        ...
        <div className="hidden lg:block lg:col-span-3 space-y-3">
          {factoryTabs.map((item) => (
            ...
            >
              ...
              {item.id === 'custom-orders' && (hasNewCustomOrders(customOrders, proposals) || unreadMessageOrderIds.length > 0) && (
                <span className="absolute right-6 top-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse" />
              )}
            </button>
          ))}
        ...
        <div className="col-span-12 lg:col-span-9 min-h-[600px]">
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === 'overview' && <FactoryAnalytics factoryId={user.id} />}
                ...
                isAddingProduct || editingProduct ? (
                  ...
                  <div className="space-y-8">
                    <div className="flex gap-4">
                      ...
                      <Button variant="outline" className="rounded-2xl"><Filter size={18} /></Button>
                    </div>
                    ...
                    <ProductList 
                      ...
                      onAdd={() => setIsAddingProduct(true)}                      onImport={async (e) => {
                        ...
                        reader.onload = async (evt) => {
                          try {
                            ...
                            const res = await fetch('/api/products/import', {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ products: data, factory_id: user.id })
                            });
              ...
              {activeTab === 'orders' && (
                <div className="space-y-8">
                  ...
                  <OrderList 
                    orders={orders.filter(o => o.product_name?.toLowerCase().includes(searchTerm.toLowerCase()) || o.id.toString().includes(searchTerm))} 
                    isLoading={isLoading} 
