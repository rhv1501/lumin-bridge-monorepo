...
const productSchema = z.object({
  buyer_price_inr: z.preprocess((val) => (val === '' || val === null || val === undefined ? undefined : Number(val)), z.number().min(0, 'Price must be 0 or more').optional()),
...
export const ProductTable = ({ products, isLoading, settings, onUpdate, onDelete }: ProductTableProps) => {
  ...
  const handleEdit = (product: Product) => {
    ...
    reset({
      buyer_price_inr: product.buyer_price_inr || parseFloat((product.factory_price_cny * parseFloat(settings.exchange_rate) * parseFloat(settings.admin_markup)).toFixed(2)),
  ...
  const onSubmit = async (data: ProductFormData) => {
    if (editingProduct) {
      await onUpdate(editingProduct, {
        ...data,
        buyer_price_inr: data.buyer_price_inr ? parseFloat(data.buyer_price_inr.toString()) : undefined
      });
      setEditingProduct(null);
    }
  };
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-zinc-200/50 dark:border-zinc-800/50 bg-zinc-50/50 dark:bg-zinc-800/20">
              ...
              <th className="p-6 text-[10px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">Performance</th>
              <th className="p-6 text-[10px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">Factory</th>
              <th className="p-6 text-[10px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">F. Price (CNY)</th>
              <th className="p-6 text-[10px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">B. Price (INR)</th>
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
            {isLoading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <tr key={i}>
                  <td className="p-6"><div className="flex items-center gap-3"><Skeleton className="w-10 h-10 rounded-xl" /><Skeleton className="w-32 h-4 rounded-full" /></div></td>
                ...
                <tr key={p.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors group">
                  ...
                  <td className="p-6 font-mono text-sm font-bold text-zinc-900">
                    {editingProduct === p.id ? (
                      ...
                      <span>₹{p.buyer_price_inr?.toLocaleString() || 'N/A'}</span>
                    )}
                  </td>
                  <td className="p-6">
                    {editingProduct === p.id ? (
                      <select {...register('status')} defaultValue={p.status} className="px-3 py-2 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white outline-none">
                  ...
                  <td className="p-6 text-right">
                    {editingProduct === p.id ? (
                      ...
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="sm" variant="outline" onClick={() => handleEdit(p)} className="rounded-full h-8 w-8 p-0">
                          <Edit size={14} />
                        </Button>
                        <Button size="sm" variant="danger" onClick={() => onDelete(p.id)} className="rounded-full h-8 w-8 p-0">
                          <Trash2 size={14} />
                        </Button>
                      </div>
                    )}
