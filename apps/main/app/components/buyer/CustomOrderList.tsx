...
import { FileText, X, Check, XCircle, MessageSquare, Download } from 'lucide-react';
...
export const CustomOrderList = ({ customOrders, proposals, isLoading, currentUser, onAcceptProposal, onRejectProposal, unreadMessageOrderIds = [], onNewRequest }: CustomOrderListProps) => {
  ...
  const getProposalsForOrder = (orderId: number) => {
    return proposals.filter(p => p.custom_order_id === orderId && p.status !== 'pending');
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {isLoading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <Card key={i} className="overflow-hidden border-0 shadow-lg bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-[2.5rem] p-6">
          ...
          customOrders.map(co => {
            ...
            return (
              <Card key={co.id} className="overflow-hidden border-0 shadow-lg hover:shadow-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-[2.5rem] transition-all duration-500 group">
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <span className={cn(
                          ...
                          co.status === 'pending' && "bg-amber-50 text-amber-600 border-amber-200/50 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20",
                          co.status === 'proposal_sent' && "bg-blue-50 text-blue-600 border-blue-200/50 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20",
                          co.status === 'accepted' && "bg-emerald-50 text-emerald-600 border-emerald-200/50 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20",
                          co.status === 'rejected' && "bg-red-50 text-red-600 border-red-200/50 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20"
                        )}>
                          {co.status.replace('_', ' ')}
                        </span>
                      </div>
                      <p className="text-sm text-zinc-600 dark:text-zinc-400 line-clamp-3 mb-8 leading-relaxed font-medium">{co.requirements}</p>
                    </div>
        ...
        {selectedCustomOrder && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 lg:p-10">
            ...
            <motion.div
              ...
              className="bg-white/95 dark:bg-zinc-900/95 backdrop-blur-2xl rounded-[3rem] w-full max-w-4xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] overflow-hidden relative z-10 border border-white/20 dark:border-zinc-800/50 flex flex-col max-h-[90vh]"
            >
              <div className="flex items-center justify-between p-8 sm:p-10 border-b border-zinc-100 dark:border-zinc-800/50 shrink-0">
                <div className="space-y-1">
                    <h2 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Custom Sourcing Details</h2>
                    <p className="text-sm text-zinc-500">Request #{selectedCustomOrder.id}</p>
                </div>
                <div className="flex items-center gap-4">
                  <Button variant={showChat ? "secondary" : "outline"} size="sm" onClick={() => setShowChat(!showChat)} className="relative h-12 rounded-full px-8">
                    <MessageSquare size={18} className="mr-2" />
                    {showChat ? 'View Proposals' : 'Direct Support Chat'}
                    {unreadMessageOrderIds.includes(selectedCustomOrder.id) && (
                      <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full border-2 border-white dark:border-zinc-900 shadow-md"></span>
                    )}
                  </Button>
                  <button 
                ...
                <div className="p-8 bg-zinc-50 dark:bg-zinc-800/40 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800/50 flex flex-col md:flex-row gap-10">
                  {selectedCustomOrder.photo && (
                    <div className="w-full md:w-48 h-64 md:h-48 bg-white dark:bg-zinc-800 rounded-3xl overflow-hidden flex-shrink-0 shadow-lg border border-white dark:border-zinc-700">
                      <img src={selectedCustomOrder.photo} alt="Req" className="w-full h-full object-cover" />
                    </div>
                  )}
                    ...
                    <div className="grid gap-6">
                      {getProposalsForOrder(selectedCustomOrder.id).length === 0 ? (
                        <div className="py-20 flex flex-col items-center justify-center text-center space-y-4">
                            ...
                            <div>
                                <h4 className="font-bold text-zinc-900 dark:text-white">Awaiting Factory Proposals</h4>
                                <p className="text-zinc-500 max-w-sm mt-1">Our global factory network is currently reviewing your requirements. You'll be notified as soon as a proposal is sent.</p>
                            </div>
                        </div>
                      ) : (
                          ...
                          <Card key={p.id} className="p-8 border-0 shadow-xl rounded-[2.5rem] bg-white/80 dark:bg-zinc-800/50 hover:bg-white dark:hover:bg-zinc-800 transition-all duration-300">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
                              ...
                              <span className={cn(
                                ...
                                p.status === 'published' && "bg-blue-50 text-blue-600 border-blue-200/50 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20",
                                p.status === 'accepted' && "bg-emerald-50 text-emerald-600 border-emerald-200/50 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20",
                                p.status === 'rejected' && "bg-red-50 text-red-600 border-red-200/50 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20"
                              )}>
                                {p.status === 'published' ? 'NEEDS REVIEW' : p.status}
                              </span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-6 mb-8">
                                ...
                                <div className="p-4 bg-emerald-50 text-emerald-700 dark:bg-emerald-500/5 dark:text-emerald-400 rounded-2xl border border-emerald-100 dark:border-emerald-500/10">
                                    <span className="text-[10px] font-bold uppercase tracking-widest opacity-60 mb-1 block">Lumina Verified</span>
                                    <span className="text-lg font-bold">Factory Secured</span>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row justify-end gap-4 pt-6 border-t border-zinc-100 dark:border-zinc-800/20">
                              {p.status === 'published' && (
                                <>
                                  <Button variant="outline" onClick={() => onRejectProposal(p.id)} className="h-14 rounded-2xl px-10 text-red-600 border-red-100 font-bold hover:bg-red-50 transition-all">
                                    <XCircle size={20} className="mr-2" />
                                    Reject Deal
                                  </Button>
                                  <Button onClick={() => onAcceptProposal(p.id)} className="h-14 rounded-2xl px-12 font-bold shadow-xl shadow-zinc-900/10 transition-all">
