  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
      {isLoading ? (
        ...
        products.map(p => (
          <Card key={p.id} className="overflow-hidden group border-0 shadow-lg hover:shadow-xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-[2rem] transition-all duration-500 flex flex-col h-full">
            <div className="aspect-[4/3] bg-zinc-100 dark:bg-zinc-800/50 relative overflow-hidden">
              {p.photo ? (
                <>
                  <img src={p.photo} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
              ...
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0">
                <Button size="sm" onClick={() => onSelect(p)} className="shadow-lg bg-white/90 dark:bg-zinc-900/90 text-zinc-900 dark:text-white rounded-full px-4">
            ...
            <div className="p-6 flex flex-col flex-1">
              <div className="flex justify-between items-start mb-3">
                <h3 className="font-semibold tracking-tight text-zinc-900 dark:text-zinc-50 line-clamp-1 text-lg">{p.name}</h3>
                {p.average_rating ? (
